 ...
 * 处理短信注册的业务
 */
public class PhoneSmsRegisterService {

    private TextView txtCountryCode;                // 包含国家码的控件
    private TextView txtPhoneNumber;                // 包含手机号控件
    private TextView txtCheckCode;                  // 包含验证码控件
    private Button btn_requireCheckCode;            // 获取验证码的按钮
    private View btn_register;                      // 注册按钮
    ...
    public PhoneSmsRegisterService(final Context context,
                                   ...
                                   View btn_register)
    {
        ...
        this.btn_register.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                countryCode = PhoneSmsRegisterService.this.txtCountryCode.getText().toString();
                countryCode = countryCode.substring(countryCode.indexOf('+') + 1);              // 解析国家码
                phoneNumber = PhoneSmsRegisterService.this.txtPhoneNumber.getText().toString();      // 获取手机号
                checkCode = PhoneSmsRegisterService.this.txtCheckCode.getText().toString();          // 获取验证码

                // 1. 判断手机号是否有效
                if (!Util.validPhoneNumber(countryCode, phoneNumber)) {
                    Util.showToast(PhoneSmsRegisterService.this.context, "请输入有效的手机号");
                    return;
                }

                // 2. 判断验证码是否为空（是否请求了验证码由上层控制）
                if (checkCode.length() == 0) {
                    Util.showToast(PhoneSmsRegisterService.this.context, "请输入验证码");
                    return;
                }

                // 3. 向TLS验证手机号和验证码
                PhoneSmsRegisterService.this.tlsService.smsRegVerifyCode(checkCode, smsRegListener);
            }
        });
        ...
        this.btn_requireCheckCode.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                // 2. 请求验证码
                PhoneSmsRegisterService.this.tlsService.smsRegAskCode(countryCode, phoneNumber, smsRegListener);
            }
        });
    }
    ...
    class SmsRegListener implements TLSSmsRegListener {
        ...
        public void OnSmsRegAskCodeSuccess(int reaskDuration, int expireDuration) {
            ...
            Util.startTimer(context, btn_requireCheckCode, "获取验证码", "重新获取", reaskDuration, 1);
        }
        ...
        public void OnSmsRegCommitSuccess(TLSUserInfo userInfo) {
            ...
            Intent intent = new Intent(context, PhoneSmsLoginActivity.class);
